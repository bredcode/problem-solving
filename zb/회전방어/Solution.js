...
let N = 0;
let enemies = [];
let selected = new Array(9).fill(-1);
let answer = 1e9;
...
function dfs(idx) {
    ...
    if (idx === N) {
        ...
        let view = 0;  // 처음에 상(위쪽) 방향을 보고 있음
        let result = 0;
        for (let i = 0; i < N; i++) {  // 1초부터 N초까지 확인
            // 선택지에 따라서 주인공의 방향 변경
            if (selected[i] === 0) {
                // 가만히 있기
            ...
            for (let j = 0; j < 4; j++) {
                let power = enemies[j][i];  // 해당 위치의 적군의 공격력
                // 앞에 존재하는 적은 공격력의 1배수
                if (view === j) {
                    result += power;
                }
